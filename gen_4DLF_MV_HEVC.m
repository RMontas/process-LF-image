...
if (~exist('4DLF_PVS_MV_HEVC', 'dir'))
    mkdir('4DLF_PVS_MV_HEVC');
end
...
%for i = 1:nfiles
    ...
    for ypos = 1:(num_MIs)
        for xpos = 1:(num_MIs)
            ...
            views_rgb_img = cat(4, views_rgb_img, squeeze(LF(offset13 + ypos, offset13 + xpos, :, :, 1:3)));
            %views_yuv_img = cat(4, views_yuv_img, rgb2ycbcr(squeeze(LF(ypos, xpos, :, :, 1:3))));
            
            if use_10bpp == 1
                %  lenslet_yuv_img = uint16(rgb2ycbcr(double(lenslet_rgb_img) / 1023) * 1023);
                views_yuv_img = cat(4, views_yuv_img, uint16(rgb2ycbcr(double(squeeze(LF(offset13 + ypos, offset13 + xpos, :, :, 1:3)))/1023)*1023));
            else
                % lenslet_yuv_img = rgb2ycbcr(lenslet_rgb_img);
                views_yuv_img = cat(4, views_yuv_img, rgb2ycbcr(squeeze(LF(offset13 + ypos, offset13 + xpos, :, :, 1:3))));
            end
    ...
    if use_10bpp == 1
        % RGB Actual writing
        for ypos = 1:num_MIs
            fileID = fopen( strcat(path, '4DLF_PVS_MV_HEVC/', name, '_', num2str(ypos),'_RGB444_10bpp.rgb'), 'w' );
            for xpos = 1:num_MIs
                aux_views_rgb_img = views_rgb_img(:,:,:,(ypos-1)*num_MIs+xpos);
                fwrite(fileID, aux_views_rgb_img(:,:,1)', 'uint16');
                fwrite(fileID, aux_views_rgb_img(:,:,2)', 'uint16');
                fwrite(fileID, aux_views_rgb_img(:,:,3)', 'uint16');
            end
            fclose(fileID);
        end
        % YUV Actual writing
        for ypos = 1:num_MIs
            fileID = fopen( strcat(path, '4DLF_PVS_MV_HEVC/', name, '_', num2str(ypos),'_YUV444_10bpp.yuv'), 'w' );
            for xpos = 1:num_MIs
                aux_views_yuv_img = views_yuv_img(:,:,:,(ypos-1)*num_MIs+xpos);
                fwrite(fileID, aux_views_yuv_img(:,:,1)', 'uint16');
                fwrite(fileID, aux_views_yuv_img(:,:,2)', 'uint16');
                fwrite(fileID, aux_views_yuv_img(:,:,3)', 'uint16');
            end
            fclose(fileID);
        end
        % YUV Actual writing
        for ypos = 1:num_MIs
            fileID = fopen( strcat(path, '4DLF_PVS_MV_HEVC/', name, '_', num2str(ypos),'_YUV420_10bpp.yuv'), 'w' );
            for xpos = 1:num_MIs
                aux_views_yuv_420_img = downsample10(views_yuv_img(:,:,:,(ypos-1)*num_MIs+xpos));
                fwrite(fileID, aux_views_yuv_420_img{1}', 'uint16');
                fwrite(fileID, aux_views_yuv_420_img{2}', 'uint16');
                fwrite(fileID, aux_views_yuv_420_img{3}', 'uint16');
            end
            fclose(fileID);
        end
        ...
        % YUV Actual writing
        for ypos = 1:num_MIs
            fileID = fopen( strcat(path, '4DLF_PVS_MV_HEVC/', name, '_', num2str(ypos),'_YUV420_8bpp.yuv'), 'w' );
            for xpos = 1:num_MIs
                aux_views_yuv_420_8_img = downsample(views_yuv_img(:,:,:,(ypos-1)*num_MIs+xpos));
                fwrite(fileID, aux_views_yuv_420_8_img{1}', 'uint8');
                fwrite(fileID, aux_views_yuv_420_8_img{2}', 'uint8');
                fwrite(fileID, aux_views_yuv_420_8_img{3}', 'uint8');
            end
            fclose(fileID);
        end
        % RGB Actual writing
