...
% REC image is converted from YUV422@10bpp to YUV444@10bpp 4DLF_VIEWS to compare with REF
...
function [] = calcMetrics_YUV422_10bpp(REF, REC, representation_type, H, W, mi_size, metadata, output_folder)
...
% process REC
if representation_type >= 2 && representation_type <= 4 %% 4DLF_PVS abd 4DLF_PVS_SCL
    ...
    if representation_type == 2
        for j = 1:mi_size
            for i = 1:mi_size
                ...
                U = fread(f, [W/2 H], 'uint16');
                V = fread(f, [W/2 H], 'uint16');
                rec_YUV422_10bpp{1,1} = double(Y');
                rec_YUV422_10bpp{2,1} = double(U');
                rec_YUV422_10bpp{3,1} = double(V');
                rec_YUV444_10bpp = upsample10_YUV422_to_YUV444(rec_YUV422_10bpp); % upsample 8bpp 444 to 420
                rec_4DLF_VIEWS(ypos,xpos,:,:,:) = uint16(double(rec_YUV444_10bpp(1:434,1:625,:))); %
            end
        end
    end
    if representation_type == 3 %% 4DLF_PVS_SCL
        for l = 1:num_Layers
            for j = 1:mi_size
                for i = 1:mi_size
                    [ypos, xpos] = find(cc_spiral == (j-1)*mi_size + i);
                    if layerMask(ypos,xpos) == l
                        Y = fread(f, [W H], 'uint8');
                        U = fread(f, [W/2 H], 'uint8');
                        V = fread(f, [W/2 H], 'uint8');
                        rec_YUV422_10bpp{1,1} = double(Y');
                        rec_YUV422_10bpp{2,1} = double(U');
                        rec_YUV422_10bpp{3,1} = double(V');
                        rec_YUV444_10bpp = upsample10_YUV422_to_YUV444(rec_YUV422_10bpp); % upsample 8bpp 444 to 420
