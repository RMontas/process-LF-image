function [LFdecC, LFdec, LFWeight, DecodeOptions, CorrectedLensletImage] = ...
    DecodeLenslet_RGB444_10bpp( LensletImage, MAT) %, LensletGridModel, DecodeOptions )
...
winame = ['/home/rmonteiro/PhD/Sequences/EPFL/Cameras/',DecodeOptions.WhiteImageInfo.Fname]; % needs to be updated to your environment
...
%LensletImage_YUV_420{1,1} = double(imread(Y));
%LensletImage_YUV_420{2,1} = double(imread(U));
%LensletImage_YUV_420{3,1} = double(imread(V));
...
LFdec = SliceXYImage( NewLensletGridModel, LensletImage, WhiteImage, DecodeOptions );
...
DecodeOptions.Gamma = 1/2.2; %% previous CFP
DecodeOptions.ColourMatrix = [  2.3620   -0.4761   -0.2880
                               -1.1726    1.6742   -0.8884
                               -0.1893   -0.1981    2.1764 ];
