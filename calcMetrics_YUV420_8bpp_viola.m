...
function [] = calcMetrics_YUV420_8bpp_viola(REF, REC, representation_type, H, W, mi_size, metadata, output_folder)
...
f = fopen(REF,'r');
Y = fread(f, [W_REF H_REF], 'uint8');
U = fread(f, [W_REF H_REF], 'uint8');
V = fread(f, [W_REF H_REF], 'uint8');
fclose(f);
ref_4DLF_MI = uint8(cat(3, Y', U', V'));
clear Y U V
ref_4DLF_VIEWS = deconstruct_lenslet_img( ref_4DLF_MI, mi_size ); % 4DLF_MI to 4DLF_VIEWS
timestamp = fix(clock);
timestamp = strcat(num2str(timestamp(4),2),num2str(timestamp(5),2),num2str(timestamp(6),2));
imwrite(ycbcr2rgb(squeeze(ref_4DLF_VIEWS((mi_size+1)/2,(mi_size+1)/2,:,:,1:3))), strcat(output_folder, 'REF_central_view_',timestamp,'.png'), 'png');
...
if representation_type == 2 %% 4DLF_PVS
    ...
    for j = 1:mi_size
        for i = 1:mi_size
            ...
            rec_4DLF_VIEWS(ypos,xpos,:,:,:) = uint8(double(rec_YUV444_8bpp(1:434,1:625,:))); % upsample 8bpp to 10bpp
        end
    end
    fclose(f);
else
    ...
    if representation_type == 0 %% LENS
        rec_RGB444_8bpp = single(ycbcr2rgb(double(rec_YUV444_8bpp) / 255));
        run('LFToolbox0.4/LFMatlabPathSetup')
        [rec_4DLF_VIEWS_RGB444_8bpp, LFdec, LFWeight, DecodeOptions, CorrectedLensletImage] = DecodeLenslet_RGB444_8bpp( rec_RGB444_8bpp, metadata);
        for i = 1:mi_size
            for j = 1:mi_size
                rec_4DLF_VIEWS(j,i,:,:,:) = uint8(rgb2ycbcr(double(squeeze(rec_4DLF_VIEWS_RGB444_8bpp(j,i,:,:,:)))) * 255);
            end
        end
    elseif representation_type == 1 %% 4DLF_MI
        rec_4DLF_VIEWS = deconstruct_lenslet_img( uint8(rec_YUV444_8bpp), mi_size ); % 4DLF_MI to 4DLF_VIEWS
    end
end
imwrite(ycbcr2rgb(squeeze(rec_4DLF_VIEWS((mi_size+1)/2,(mi_size+1)/2,:,:,1:3))), strcat(output_folder, 'REC_central_view_',timestamp,'.png'), 'png');

[PSNR_Y, PSNR_U, PSNR_V, PSNR_YUV, PSNR_Y_mean, PSNR_U_mean, PSNR_V_mean, PSNR_YUV_mean] = ComputePSNR_YUV444_8bpp(rec_4DLF_VIEWS, ref_4DLF_VIEWS);
